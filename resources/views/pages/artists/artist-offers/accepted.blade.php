                                ...
                                <div class="col-xs-12 remove_offer m-b" id="remove_offer-{{$sendOffer->id}}">
                                    <div class="item r Item" data-id="item-{{$sendOffer->id}}">
                                        <div class="item-media">
                                            <div class="item-media-content" style="background-image: url({{ !empty($sendOffer->userCurator->profile) ? (strpos($sendOffer->userCurator->profile, 'http') !== false ? $sendOffer->userCurator->profile : URL('/').'/uploads/profile/'.$sendOffer->userCurator->profile) : asset('images/profile_images_icons.svg') }});"></div>
                                        </div>
                                        ...
                                        <div class="item-info">
                                            ...
                                            <div class="m-t-sm offerAlternative">
                                                <div><span style="color:#02b875 !important">Offer Type: </span><span class="btn btn-xs white">{{ $sendOffer->curatorOfferTemplate->offerType->name ?? '----' }}</span></div>
                                                <div><span style="color:#02b875 !important">Expiry Date: </span><span class="btn btn-xs white">{{ $sendOffer->expiry_date ? \Carbon\Carbon::parse($sendOffer->expiry_date)->format('M d Y') : '' }}</span></div>
                                            </div>
                                            ...
                                            <div class="m-t-sm campaignBtn" style="display:flex; gap: 10px; align-items: center; flex-wrap: wrap;">
                                                ...
                                                @elseif($sendOffer->status == 'delivered')
                                                    @if(!\App\Models\CuratorRating::where('offer_id', $sendOffer->id)->exists())
                                                        <button type="button" class="btn btn-xs btn-primary rounded-pill" data-toggle="modal" data-target="#rateModal{{$sendOffer->id}}">Review & Rate</button>
...
@section('page-script')
<script>
    function OfferShow(id) { $("#form-offer" + id).submit(); }
</script>
@endsection
